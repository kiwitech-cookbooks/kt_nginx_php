# Encoding: utf-8
# Cookbook Name:: kt_nginx_php
# Recipe:: default
#
# Copyright 2016, Kiwi Technologies Pvt, Ltd.
#
# All rights reserved - Do Not Redistribute
#

# We only support 5.5 and 5.6
if node['kt_nginx_php']['php_version'] != '5.5' && node['kt_nginx_php']['php_version'] != '5.6'
  Chef::Application.fatal!("PHP version #{node['kt_nginx_php']['php_version']} is not supported")
end
...
template "#{node['nginx']['dir']}/sites-available/default.conf" do
  source node['kt_nginx_php']['nginx']['default_site']['template']
  cookbook node['kt_nginx_php']['nginx']['default_site']['cookbook']
  owner 'root'
  group 'root'
  mode '0644'
  action :create
...
template '/var/www/default/index.html' do
  ...
  mode '0644'
  action :create
  only_if { node['kt_nginx_php']['index_file'] }
end


# Configuration for info.php page
template '/var/www/default/info.php' do
  source 'info.php.erb'
  owner 'ubuntu'
  group 'ubuntu'
  mode '0644'
...
nginx_site 'default.conf' do
  enable node['kt_nginx_php']['nginx']['default_site']['enable']
end
...
if platform_family?('rhel')
  ...
  apt_repository "php-#{node['kt_nginx_php']['php_version']}" do
    uri          php_fpm[node['platform_family']][node['kt_nginx_php']['php_version']]['repo']
...
node.default['php-fpm']['package_name'] = php_fpm[node['platform_family']][node['kt_nginx_php']['php_version']]['php_fpm_package']
node.default['php-fpm']['service_name'] = php_fpm[node['platform_family']][node['kt_nginx_php']['php_version']]['service']
...
php_fpm_pool 'default' do
  enable node['kt_nginx_php']['php-fpm']['default_pool']['enable']
end

# PHP
# Set the correct php packages to install
if node['kt_nginx_php']['php_packages_install']['enable']
  node.default['php']['packages'] = php_fpm[node['platform_family']][node['kt_nginx_php']['php_version']]['php_packages']
